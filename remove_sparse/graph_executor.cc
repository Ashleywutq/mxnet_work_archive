...
#include "../operator/subgraph/mkldnn/mkldnn_conv.h"
...
Executor *Executor::SimpleBind(nnvm::Symbol symbol,
  ...
  //remove sparse
  if (!need_grad && !dmlc::GetEnv("MXNET_DISABLE_FUSION_ALL", 0)) {
  nnvm::Symbol ret = symbol.Copy();
  nnvm::Graph g;
  g.outputs = ret.outputs;
  g.attrs["mxnet_version"] =
      std::make_shared<nnvm::any>(static_cast<int>(MXNET_VERSION));
  g = ApplyPass(std::move(g), "PartitionGraphRemoveSparse");
  ...
  g.attrs["mxnet_version"] =
      std::make_shared<nnvm::any>(static_cast<int>(MXNET_VERSION));
  op::sg::SubgraphPropertyPtr property =
      std::make_shared<op::sg::SgMKLDNNConvProperty>();
