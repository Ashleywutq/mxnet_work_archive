...
#include <cstdio>
#include <ctime>
#include "../../../3rdparty/mkldnn/external/mklml_lnx_2018.0.3.20180406/include/mkl_trans.h"
#include <type_traits>
#include <cstdint>
#include <xmmintrin.h>
...
struct TransposeParam : public dmlc::Parameter<TransposeParam> {
 ...
 //function to test mkl
inline void Testmklfunc(){
  float* A = (float*) malloc( 48*sizeof( float ) );
  float* B = (float*) malloc( 48*sizeof( float ) );
  ...
  mkl_somatcopy2('r', 't', 8, 3, 1, A,6,2,B,16,2);
...
//function for mshadow 4 dimension transpose
template<typename xpu, typename DType>
void MKLtranspose(mshadow::Tensor<xpu, 4, DType>& in, mshadow::Tensor<xpu, 4, DType>& out, const TShape& axes){
  out = transpose(in, axes.get<4>());
}



//function for mkl 4 dimension transpose (0,2,1,3) for case float
template<> inline
void MKLtranspose<cpu,float>(mshadow::Tensor<cpu, 4, float>& in, mshadow::Tensor<cpu, 4, float>& out, const TShape& axes){
  mshadow::Shape<4> src_shape = in.shape_;
  mshadow::Shape<4> dst_shape = out.shape_;
  mshadow::Shape<4> src_stride;
  mshadow::Shape<4> dst_stride;

  //check input shape
  //std::cout << src_shape[0] << "," << src_shape[1] << "," << src_shape[2] << ","
  //         << src_shape[3] << std::endl;

  src_stride[3] = 1;
  dst_stride[3] = 1;
  for (int i = 2; i >= 0; --i) {
    src_stride[i] = src_shape[i + 1] * src_stride[i + 1];
    dst_stride[i] = dst_shape[i + 1] * dst_stride[i + 1];
    }

  size_t lda = src_shape[2]*src_shape[3];
  size_t ldb = dst_shape[2]*dst_shape[3];

  #pragma omp parallel for
  for (index_t i = 0; i < dst_shape[0];i++){
    int strideindex = i * dst_stride[0];
    for (index_t j = 0; j < dst_shape[3];j++) {
      //transpose by slices
      mkl_somatcopy2('r', 't', dst_shape[2], dst_shape[1], 1, in.dptr_+strideindex,lda,src_shape[3],
                  out.dptr_+strideindex,ldb,dst_shape[3]);
      strideindex++;
    }
  }
    ...
    /*
  //check accuracy
  std::cout << in.dptr_[1 * src_stride[0] + (src_shape[1]-1)*src_stride[1]+(src_shape[2]-1)*src_stride[2]] <<":"
            << out.dptr_[1 * dst_stride[0] + (dst_shape[1]-1)*dst_stride[1]+(dst_shape[2]-1)*dst_stride[2]] <<"--"
            << in.dptr_[1 * src_stride[0] + (src_shape[1]-1)*src_stride[1]+(src_shape[2]-1)*src_stride[2]+2] <<":"
            << out.dptr_[1 * dst_stride[0] + (dst_shape[1]-1)*dst_stride[1]+(dst_shape[2]-1)*dst_stride[2]+2]
            << std::endl;
  */
}

//function for specific 4 dimension transpose using memcpy and intrinsics
template<typename xpu, typename DType>
void S4Dtranspose(mshadow::Tensor<xpu, 4, DType>& in, mshadow::Tensor<xpu, 4, DType>& out, const TShape& axes){
  out = transpose(in, axes.get<4>());
}

//function for specific 4 dimension transpose (0,2,1,3) using memcpy and intrisics for case float
template<> inline
void S4Dtranspose<cpu,float>(mshadow::Tensor<cpu, 4, float>& in, mshadow::Tensor<cpu, 4, float>& out, const TShape& axes){
  mshadow::Shape<4> daxes = axes.get<4>();
  mshadow::Shape<4> src_shape = in.shape_;
  mshadow::Shape<4> dst_shape = out.shape_;
  mshadow::Shape<4> src_stride;
  mshadow::Shape<4> dst_stride;
  mshadow::Shape<4> dstsrc_stride;
  ...
  for (int i = 2; i >= 0; --i) {
    ...
    for (int i = 0; i<4;i++){
      index_t a = daxes[i];
      dstsrc_stride[i] = src_stride[a];
    }


    #pragma omp parallel for
        for (index_t i = 0; i < dst_shape[0]; i++) {
          int outindex = i*dst_stride[0];
          int inindexi = outindex;
          for (index_t j = 0; j < dst_shape[1]; j++) {
            int inindexj = inindexi + j * dstsrc_stride[1];
            for (index_t m = 0; m < dst_shape[2]; m++) {
              int inindexm = inindexj + m * dstsrc_stride[2];
              int copy_stride = dst_shape[3];
              //memcpy(out.dptr_+outindex, in.dptr_+inindexm, sizeof(float) * copy_stride);
              ///copy using intrinsics
              index_t k;
              for (k = 0; k < src_shape[3] / 4; k += 4) {
                __m128 row1 = _mm_load_ps(in.dptr_ + inindexm + k);
                _mm_store_ps(out.dptr_ + outindex + k, row1);
              }

              for (; k < src_shape[3];k++) {
                out.dptr_[outindex + k] = in.dptr_[inindexm + k];
              }
              outindex += copy_stride;
            }
           }
        }
...
//function for (0,2,1,3) 4d transpose using block
template<typename xpu, typename DType>
void Btranspose(mshadow::Tensor<xpu, 4, DType>& in, mshadow::Tensor<xpu, 4, DType>& out, const TShape& axes){
  out = transpose(in, axes.get<4>());
}

//function for (0,2,1,3) 4d transpose using block
template<> inline
void Btranspose<cpu,float>(mshadow::Tensor<cpu, 4, float>& in, mshadow::Tensor<cpu, 4, float>& out, const TShape& axes){
  mshadow::Shape<4> src_shape = in.shape_;
  mshadow::Shape<4> dst_shape = out.shape_;
  mshadow::Shape<4> src_stride;
  mshadow::Shape<4> dst_stride;
  ...
  index_t n = src_shape[1];
  index_t m = src_shape[2];
  index_t block_size = 2;

 #pragma omp parallel for
        for (index_t k = 0; k < dst_shape[0]; k++) {
          index_t indexstride = k*dst_stride[0];

    //transpose 3D matrix by block
    for(index_t i=0; i<n; i+=block_size) {
      index_t max_i = (i+block_size) < n ? block_size : (n-i);
      index_t A = indexstride + i * src_stride[1];
      index_t B = indexstride + i * dst_stride[2];
      for (index_t j = 0; j < m; j += block_size) {
        index_t max_j = (j + block_size) < m ? block_size : (m - j);
        // block operation
        for (index_t bi = 0; bi < max_i; bi++) {
          index_t AA = A + bi * src_stride[1];
          index_t BB = B + bi * dst_stride[2];
          for (index_t bj = 0; bj < max_j; bj++) {

            for (index_t c = 0; c < src_shape[3]; c++) {
              out.dptr_[BB + c] = in.dptr_[AA + c];
            }
...
//function for general 4d transpose
template<typename xpu, typename DType>
void PMDtranspose(mshadow::Tensor<xpu, 4, DType>& in, mshadow::Tensor<xpu, 4, DType>& out, const TShape& axes){
  mshadow::Shape<4> daxes = axes.get<4>();
  mshadow::Shape<4> src_shape = in.shape_;
  mshadow::Shape<4> dst_shape = out.shape_;
  mshadow::Shape<4> src_stride;
  ...
  for (int i = 2; i >= 0; --i) {
    ...
    #pragma omp parallel for
        for (index_t i = 0; i < dst_shape[0]; i++) {
          int outindex = i*dst_stride[0]-1;
          int inindexi = i * dstsrc_stride[0];
          for (index_t j = 0; j < dst_shape[1]; j++) {
            int inindexj = inindexi + j * dstsrc_stride[1];
            for (index_t m = 0; m < dst_shape[2]; m++) {
              int inindexm = inindexj + m * dstsrc_stride[2]-dstsrc_stride[3];
              for (index_t k = 0; k < dst_shape[3]; k++) {
                outindex += 1;
                inindexm += dstsrc_stride[3];
                out.dptr_[outindex] = in.dptr_[inindexm];
              }
             }
           }
        }
...
//function for general 4d transpose
template<typename xpu, typename DType>
void MDtranspose(mshadow::Tensor<xpu, 4, DType>& in, mshadow::Tensor<xpu, 4, DType>& out, const TShape& axes){
  mshadow::Shape<4> daxes = axes.get<4>();
  mshadow::Shape<4> src_shape = in.shape_;
  mshadow::Shape<4> dst_shape = out.shape_;
  mshadow::Shape<4> src_stride;
...
void TransposeImpl(RunContext ctx, const TBlob& src, const TBlob& ret,
                   const TShape& axes) {
  ...
  MSHADOW_TYPE_SWITCH(ret.type_flag_, DType, {
    switch (axes.ndim()) {
     ...
     case 4: {
      ...
      //out = transpose(in, axes.get<4>());
      MDtranspose<xpu, DType>(in, out, axes);
      //PMDtranspose<xpu, DType>(in, out, axes);
      //S4Dtranspose<xpu, DType>(in, out, axes);
      //Testmklfunc();
      //MKLtranspose<xpu, DType>(in, out, axes);
      //Btranspose<xpu, DType>(in, out, axes);
      break;
     }
     case 5: {
