...
void ForwardEx(const OpStatePtr& state, const OpContext& ctx,
  ...
  //convert mkldnn data format to default format
  std::vector<NDArray> new_inputs;
  std::vector<NDArray*> new_inputs_ptrs;
  for (size_t i = 0; i < inputs.size(); ++i) {
    NDArrayStorageType stype = inputs[i].storage_type();
#if MXNET_USE_MKLDNN == 1
    if (kUndefinedStorage == stype || kDefaultStorage == stype) {
      NDArray* tmp_ptr = new NDArray();
      *tmp_ptr = inputs[i].Reorder2Default();
      new_inputs.push_back(*tmp_ptr);
      new_inputs_ptrs.push_back(tmp_ptr);
    } else {
  ...
  for (size_t i = 0; i < params.num_args; ++i) {
    NDArray* nd;
    AllocateNDArrayCopy(&nd,new_inputs, i, dev_id);
    cpys.push_back(*nd);
  ...
  for (size_t i = 0; i < params.num_auxs; ++i) {
    ...
    NDArray* nd;
    AllocateNDArrayCopy(&nd, new_inputs, idx, dev_id);
    cpys.push_back(*nd);
    ptrs.push_back(reinterpret_cast<void*>(nd));
    tags.push_back(4);
  }
  ...
  CustomOperator::Get()->Push(
      [=]() {
        ...
        //delete reordered new_inputs
        for (auto i : new_inputs_ptrs)
          delete i;
      },
